...
class ComplexNumber(RealNumber):
    ...
    def __str__(self):
        if self.imaginary < 0:
            return f"{self.number} - {(-1) * self.imaginary}i"
...
class ScienceExam(Exam):
    ...
    def examSyllabus(self):
        ...
        if len(self.sub) == 2:
            return f"{super().examSyllabus()}, {self.sub[0]}, {self.sub[1]}"
        
        elif len(self.sub) == 3:
            return f"{super().examSyllabus()}, {self.sub[0]}, {self.sub[1]}, {self.sub[2]}"
    
    def examParts(self):
        
        if len(self.sub) == 2:
            return f"{super().examParts()}Part 3 - {self.sub[0]} \nPart 4 - {self.sub[1]}"
...
class PokemonExtra(PokemonBasic):
    ...
    def get_move(self):
        if self.other_move != None:
            return f"{super().get_move()} \nOther move: {self.other_move[0]}, {self.other_move[1]}"
        else:
...
class Charmander(Pokemon):
    def __init__(self, p = "Charmander", pokemon_type = "Fire", pokemon_weakness = "Water, Ground"):
        super().__init__(p)
        self.pokemon_type = pokemon_type
        self.pokemon_weakness = pokemon_weakness
